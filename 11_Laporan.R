...
# Cari kolom demografis secara otomatis
gender_col <- names(data_raw)[grepl("Jenis.*Kelamin|Gender|Sex", names(data_raw), ignore.case = TRUE) &
                                !grepl("angka", names(data_raw), ignore.case = TRUE)][1]
edu_col <- names(data_raw)[grepl("Tingkat.*Pendidikan|Education", names(data_raw), ignore.case = TRUE) &
                             !grepl("angka", names(data_raw), ignore.case = TRUE)][1]
usia_col <- names(data_raw)[grepl("^USIA$", names(data_raw), ignore.case = TRUE)][1]
...
if(file.exists("output/tables/14_TIRT_FitIndices.csv")) {
  ...
  print(fit_tirt, row.names = FALSE)
  cat("\nTIRT menghasilkan trait estimates yang lebih valid untuk data forced-choice.\n\n")
} else if(file.exists("output/tables/14_TIRT_Status.csv")) {
  status_tirt <- read.csv("output/tables/14_TIRT_Status.csv")
  cat("Status Thurstonian IRT:\n")
  print(status_tirt, row.names = FALSE)
}

cat("==============================================================================\n")
cat("9. NETWORK ANALYSIS\n")
cat("==============================================================================\n\n")
...
cat("==============================================================================\n")
cat("10. NORMA PENYEKORAN\n")
cat("==============================================================================\n\n")

if(file.exists("output/tables/29_Norma_RawScore.csv")) {
  norma <- read.csv("output/tables/29_Norma_RawScore.csv")
  cat("Norma Raw Score (Skor Mentah):\n\n")
  cat("Tabel norma tersedia di file output/tables/29_Norma_RawScore.csv\n")
  cat("(Menampilkan 5 baris pertama)\n")
  print(head(norma), row.names = FALSE)
  cat("\n")
}
...
cat("==============================================================================\n")
cat("11. KESIMPULAN DAN REKOMENDASI\n")
cat("==============================================================================\n\n")

cat("KESIMPULAN:\n\n")
cat("1. Instrumen EPPS telah dianalisis secara komprehensif menggunakan berbagai\n")
cat("   pendekatan IRT modern (2PL, 3PL, MIRT, TIRT, GRM).\n\n")
cat("2. Reliabilitas instrumen bervariasi. Aspek dengan reliabilitas rendah perlu perhatian.\n\n")
cat("3. Analisis Network memberikan wawasan baru tentang dinamika antar trait.\n\n")

cat("\nREKOMENDASI:\n\n")
cat("1. Gunakan skor berbasis IRT (Theta) untuk presisi lebih tinggi.\n")
cat("2. Perhatikan aspek dengan reliabilitas < 0.70 untuk revisi item.\n")
cat("3. Gunakan norma yang telah dibuat untuk interpretasi hasil tes.\n\n")
...
summary_table <- data.frame(
  ...
  Output_Files = c(
    "4 tabel", "3 tabel + 30 plot", "3 tabel + 30 plot",
    "5 tabel + 1 plot", "6 tabel + 1 plot", "5 tabel + 2 plot",
    "Multiple files", "6 tabel", "15+ plot"
  )
)
